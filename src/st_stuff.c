...
//For CTF flag timers
//Updated by p_mobj.c for draw CTF timer hud
int redflagFuse = 0;
int blueflagFuse = 0;
...
static void ST_drawCTFHUD(void)
{
	...
	//If one flag has been dropped, show the timer in the screen
	char buf[8*sizeof(int)+1];
	if (redflagFuse)
		V_DrawCenteredString(266, STRINGY(180), V_YELLOWMAP, itoa(redflagFuse/TICRATE, buf, 10));
	if (blueflagFuse)
		V_DrawCenteredString(298, STRINGY(180), V_YELLOWMAP, itoa(blueflagFuse/TICRATE, buf, 10));
}
