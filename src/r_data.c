...
static byte *R_GenerateTexture(int texnum)
{
	...
	texturememory += blocksize;
	block = Z_Malloc(blocksize, PU_STATIC, (void*)&texturecache[texnum]);
...
static void R_InitColormaps(void)
{
	...
	colormaps = Z_Malloc(W_LumpLength (lump), PU_STATIC, NULL); //Z_MallocAlign(W_LumpLength (lump), PU_STATIC, NULL, 16);
...
int R_ColormapNumForName(char *name)
{
	...
	extra_colormaps[num_extra_colormaps].colormap = Z_Malloc(W_LumpLength (lump), PU_LEVEL, NULL); //Z_MallocAlign (W_LumpLength (lump), PU_LEVEL, NULL, 16);
...
#define ABS2(x) ((x) < 0 ? -(x) : (x))
	if (rendermode == render_soft)
	{
		colormap_p = Z_Malloc((256 * 34) + 10, PU_LEVEL, NULL); //Z_MallocAlign((256 * 34) + 10, PU_LEVEL, NULL, 16);
