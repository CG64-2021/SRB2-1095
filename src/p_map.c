...
boolean P_CheckPosition(mobj_t *thing, fixed_t x, fixed_t y)
{
	...
	if (newsubsec->sector->ffloors)
	{
		...
		for (rover = newsubsec->sector->ffloors; rover; rover = rover->next)
		{
			...
			{
				if (!(thing->player && !thing->player->nightsmode && ((((thing->player->charflags & SF_RUNONWATER) && !thing->player->powers[pw_tailsfly] && sectors[rover->secnum].special != 983 && (thing->momz <= 0 && thing->momz > -10*FRACUNIT) && !thing->player->mfjumped) || thing->player->powers[pw_super]) && thing->ceilingz-*rover->topheight >= thing->height)
...
static void P_FakeZMovement(mobj_t *mo)
{
	...
	if (mo->subsector->sector->ffloors)
	{
		...
		for (rover = mo->subsector->sector->ffloors; rover; rover = rover->next)
		{
			...
			if ((!(rover->flags & FF_SOLID || rover->flags & FF_QUICKSAND) && !(mo->player && !mo->player->nightsmode && ((((mo->player->charflags & SF_RUNONWATER) && !mo->player->powers[pw_tailsfly] && sectors[rover->secnum].special != 983 && (mo->momz <= 0 && mo->momz > -10*FRACUNIT) && !mo->player->mfjumped) || mo->player->powers[pw_super]) && mo->ceilingz-*rover->topheight >= mo->height)  && (rover->flags & FF_SWIMMABLE) && !mo->player->mfspinning && mo->player->speed > mo->player->runspeed && /*mo->ceilingz - *rover->topheight >= mo->height &&*/ mo->z < *rover->topheight + 30*FRACUNIT && mo->z > *rover->topheight - 30*FRACUNIT)))
				continue;
